...
class Elevator:
    ...
    def check_cont_up(self, all_pass) :
        for p in all_pass:
            if not p.done and (p.src_floor > self.cur_floor or (p.src_floor == self.cur_floor and p.direction == "U")) :
                return True
        for cp in self.register_list:
            if cp.dest_floor > self.cur_floor:
                return True
        return False

    def check_cont_down(self, all_pass) :
        for p in all_pass:
            if not p.done and p.src_floor < self.cur_floor or (p.src_floor == self.cur_floor and p.direction == "D"):
                return True
        for cp in self.register_list:
            if cp.dest_floor < self.cur_floor:
                return True
    ...
    def move2(self, all_pass):
        ...
        if self.direction == "U":
            ...
            if self.cur_floor >= self.num_floors or not self.check_cont_up(all_pass):
                self.direction = "D"
        else:
            self.cur_floor -= 1
            # start moving up if no one below and no one wants to go below
            if self.cur_floor <= 1 or not self.check_cont_down(all_pass):
                self.direction = "U"
